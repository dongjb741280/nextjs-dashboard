...
export async function fetchRevenue() {
  ...
  try {
    ...
    const data = Array.isArray(result) ? result : (result.rows || result);
...
export async function fetchCardData() {
  ...
  try {
    ...
    // Handle different return formats (array, {rows: [...]}, or direct result)
    ...
    const invoiceCount = getRows(results[0]);
    const customerCount = getRows(results[1]);
    const invoiceStatus = getRows(results[2]);
